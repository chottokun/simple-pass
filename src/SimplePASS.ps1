...
$localeFile = Join-Path (Join-Path $scriptDir "Locales") "$Language.psd1"
if (-not (Test-Path $localeFile)) {
    $localeFile = Join-Path (Join-Path $scriptDir "Locales") "en.psd1"
}
$resObj = Import-PowerShellDataFile -Path $localeFile
$res = [PSCustomObject]$resObj
